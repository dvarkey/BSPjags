...
#'
#' @examples
writeBSPmodel <- function(rprior=0.2000379,split=FALSE){

rval=paste0('r ~ dlnorm(',round(log(rprior)-(0.5*(1/3.252)),4),',3.252)') 
rvalsplit=paste0('r1 ~ dlnorm(',round(log(rprior)-(0.5*(1/3.252)),4),',3.252)') 
part1 =  "model
{
...
part2split= "# prior distribution of K based on EPP 100,30 
K~dlnorm(4.562,11.6)
...
# State equation - SP Model. 
for (t in 2:(34))   { 
Pm[t] <- log(max(P[t-1] + r1*P[t-1]*(1-P[t-1]) - L[t-1]/K, 0.0001))
P[t] ~ dlnorm(Pm[t], isigma2)#I(0.001,5)
P.res[t]<-log(P[t])-Pm[t]
}
for (t in 35:(54))   { 
...
if(split==TRUE){WF_model=paste (part1,rvalsplit,rval,part2split,sep='\n')}
